...
fun rememberManagedExoPlayer(
    ...
    /**
     * Optional replacement for the default OkHttp-backed data source, e.g. the mobile
     * flavor wraps it in the download cache so downloaded episodes play from disk.
     */
    dataSourceFactoryOverride: ((StreamInfo) -> DataSource.Factory)? = null
): Pair<ExoPlayer, ManagedPlayerState> {
    val context = LocalContext.current
    val state = remember { ManagedPlayerState() }

    val dataSourceFactory = remember(streamInfo) {
        dataSourceFactoryOverride?.invoke(streamInfo)
